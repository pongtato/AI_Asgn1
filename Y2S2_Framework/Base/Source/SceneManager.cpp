...
static Vector3 prevPos(0, 0, 0), currentPos(0, 0, 0);
...
void CSceneManager::Init()
{
	...
	meshList[GEO_WARRIOR] = MeshBuilder::GenerateOBJ("Warrior", "OBJ//AI/Warrior.obj");
	meshList[GEO_WARRIOR]->textureID = LoadTGA("Image//AI/Warrior.tga");

	meshList[GEO_SWORD] = MeshBuilder::GenerateOBJ("Sword", "OBJ//AI/Sword.obj");
	meshList[GEO_SWORD]->textureID = LoadTGA("Image//AI/Weapons.tga");

	meshList[GEO_SHIELD] = MeshBuilder::GenerateOBJ("Shield", "OBJ//AI/Shield.obj");
	meshList[GEO_SHIELD]->textureID = LoadTGA("Image//AI/Weapons.tga");

	meshList[GEO_MAGE] = MeshBuilder::GenerateOBJ("Mage", "OBJ//AI/Mage.obj");
	meshList[GEO_MAGE]->textureID = LoadTGA("Image//AI/Mage.tga");

	meshList[GEO_STAFF] = MeshBuilder::GenerateOBJ("Staff", "OBJ//AI/Staff.obj");
	meshList[GEO_STAFF]->textureID = LoadTGA("Image//AI/Weapons.tga");

	meshList[GEO_HEALER] = MeshBuilder::GenerateOBJ("Healer", "OBJ//AI/Healer.obj");
	meshList[GEO_HEALER]->textureID = LoadTGA("Image//AI/Healer.tga");
...
void CSceneManager::Update(double dt)
{
	...
	angle += dt;
	//currentPos.Set(sin(angle) * 5.f, 0, cos(angle) * 5.f);
...
void CSceneManager::RenderGround()
{
	...
	for (int i = 0; i < m_cSpatialPartition->GetxNumOfGrid(); ++i)
	{
		...
		{
			modelStack.PushMatrix();
			modelStack.Translate(m_cSpatialPartition->xGridSize*(i+0.5f), -m_cSpatialPartition->yGridSize*(j+0.5f), 0.0f);
...
/********************************************************************************
Start rendering process by calling pre-rendering
********************************************************************************/
void CSceneManager::PreRendering(Mtx44 transform, bool enableLight)
{
	modelStack.PushMatrix();
	modelStack.MultMatrix(transform);
