...
void CSceneNode::Draw(CSceneManager* theSceneManager)
{
	if (theTransform)
	{
		...
		if (theModel)
		{
			theSceneManager->RenderMesh(theModel->getMesh(), true);
			//theModel->Draw();
		}
...
Vector3 CSceneNode::GetTopLeft(void)
{
	...
	{
		Vector3 translate;
		theTransform->GetOffset(translate.x, translate.y, translate.z);
		return (theTransform->GetTransform() * theModel->GetTopLeft() + translate);
	}
}
...
Vector3 CSceneNode::GetBottomRight(void)
{
	...
	else
	{
		Vector3 translate;
		theTransform->GetOffset(translate.x, translate.y, translate.z);
		return (theTransform->GetTransform() * theModel->GetBottomRight() + translate);
	}
}
